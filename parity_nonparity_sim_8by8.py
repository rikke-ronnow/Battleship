...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 20 21:03:27 2022

@author: shimin
"""

# hunting strategy for both parity and non-parity strategy for 8x8 board
   ...
   for rowindex, row in enumerate(board):
       if 2 in row:
           ...
           for colindex in i[0]: 
               ...
               elif board[rowindex][np.min([7,colindex+1])] == 0:
                   return rowindex, np.min([7,colindex+1])
               elif board[np.max([0,rowindex-1])][colindex] == 0:
                   return np.max([0,rowindex-1]), colindex
               elif board[np.min([7,rowindex+1])][colindex] == 0:
                   return np.min([7,rowindex+1]), colindex
               
   while True:
       rowindex=random.randint(0,7)
       colindex=random.randint(0,7)
...
for time in range(100):
    ...
    for turn in range(turns):
         ...
         guess_coord=parity(guess_board)
...
np.savetxt('parity_8x8.txt',turns)    
...
for time in range(100):
    ...
    for turn in range(turns):
         ...
         guess_coord=non_parity(guess_board)
...
np.savetxt('nonparity_8x8.txt',turns)  